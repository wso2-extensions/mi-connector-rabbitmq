...
import static org.wso2.integration.connector.utils.RabbitMQConstants.SET_REQUEUE_ON_ROLLBACK;
/**
 * This class represents the operation to requeue a RabbitMQ message.
 * It sets the decision in the message context to requeue the message
 * on rollback, ensuring that the message is returned to the queue for
 * reprocessing. This class is part of the WSO2 Integration framework
 * and extends the `AbstractConnectorOperation` to provide custom behavior
 * for message handling.
 */
public class RequeueMessage extends AbstractConnectorOperation {

    /**
     * Executes the operation to requeue a RabbitMQ message. This method sets the
     * decision in the message context to requeue the message on rollback, allowing
    ...
    public void execute(MessageContext messageContext, String responseVariable, Boolean overwriteBody) {
        ...
        try {
            // Set the decision in the message context to requeue the message on rollback
            RabbitMQUtils.setDecision(messageContext, SET_REQUEUE_ON_ROLLBACK);
            ...
            String errorDetail = "Error occurred while performing rabbitmq:requeue operation with "
